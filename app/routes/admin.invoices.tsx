...
import {
    getInvoices, getSites, createInvoice, updateInvoice, markInvoicePaid, deleteInvoice,
    getAttachments, createAttachment, deleteAttachment, type Attachment,
} from "~/lib/db";
...
export async function loader({ request, context }: Route.LoaderArgs) {
    ...
    const [invoices, sites] = await Promise.all([ getInvoices(db, { siteId }), getSites(db) ]);
...
export async function action({ request, context }: Route.ActionArgs) {
    ...
    if (intent === "create") {
        ...
        for (const file of files) {
            ...
            try {
                ...
                await createAttachment(db, { entityType: "invoice", entityId: invoiceId, fileName: file.name, fileType: file.type, fileSize: file.size, r2Key: key });
    ...
    if (intent === "paid")   { await markInvoicePaid(db, Number(form.get("id"))); return redirect("/admin/invoices"); }
    if (intent === "delete") { await deleteInvoice(db, Number(form.get("id")));   return redirect("/admin/invoices"); }
    if (intent === "deleteAttachment") {
        const r2Key = await deleteAttachment(db, Number(form.get("attachmentId")));
        if (r2Key) { try { await env.UPLOADS.delete(r2Key); } catch (e) { console.error(e); } }
        return redirect("/admin/invoices");
    }
    return null;
}
...
function InvoiceRow({
    ...
    return (
        <>
            <tr className={`transition-colors ${isOverdue ? "bg-red-50/50 dark:bg-red-950/10" : "hover:bg-gray-50 dark:hover:bg-gray-800/50"}`}>
                ...
                <td className="px-6 py-4">
                    <div className="flex items-center justify-end gap-2">
                        ...
                        <button type="button" onClick={() => setEditing((v) => !v)}
                            className="text-gray-400 hover:text-blue-600 transition-colors" title="Editar fatura">
                            <Pencil size={14} />
                        </button>
                        {inv.status === "pending" && (
                            <Form method="post">
                                ...
                                <button type="submit" title="Marcar como pago" className="text-green-500 hover:text-green-700 transition-colors">
                                    <CheckCheck size={15} />
                                </button>
                            </Form>
                        )}
                ...
                <tr className="bg-blue-50/50 dark:bg-blue-950/10">
                    <td colSpan={7} className="px-6 py-4">
                        <Form method="post" className="flex flex-wrap gap-3 items-end">
                            ...
                            <input type="hidden" name="id" value={inv.id} />
                            <div><label className="block text-xs font-medium mb-1 text-gray-500">Descrição</label>
                                <input name="description" defaultValue={inv.description} required
                                    className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                            <div><label className="block text-xs font-medium mb-1 text-gray-500">Valor (€)</label>
                                <input name="amount" type="number" step="0.01" min="0.01" defaultValue={inv.amount} required
                                    className="w-28 px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                            <div><label className="block text-xs font-medium mb-1 text-gray-500">Data limite</label>
                                <input name="dueDate" type="date" defaultValue={inv.due_date?.slice(0, 10) ?? ""}
                                    className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                            <div><label className="block text-xs font-medium mb-1 text-gray-500">Estado</label>
                            ...
                            <div className="flex gap-2">
                                <button type="submit" className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-lg transition-colors">Guardar</button>
            ...
            {open && attachments.length > 0 && (
                <tr className="bg-gray-50/80 dark:bg-gray-800/40">
                    <td colSpan={7} className="px-6 py-3">
                        <Attachments attachments={attachments} entityType="invoice" entityId={inv.id} canDelete />
                    </td>
                </tr>
            )}
        </>
    ...
    return (
        <div className={`bg-white dark:bg-gray-900 border rounded-xl p-4 ${
            ...
            <div className="flex items-start justify-between gap-2 mb-1">
                <div>
                    <p className="font-semibold text-sm">{inv.site_name}</p>
                    <p className="text-xs text-gray-400">{inv.owner_name ?? "—"}</p>
                </div>
                <StatusBadge status={inv.status} />
            </div>
            <p className="text-sm text-gray-600 dark:text-gray-300 mb-2">{inv.description}</p>
            ...
            <div className="flex flex-wrap gap-2">
                {attachments.length > 0 && (
                    <button type="button" onClick={() => setOpen((v) => !v)}
                        className="flex items-center gap-1 text-xs text-gray-400 hover:text-blue-600 transition-colors border border-gray-200 dark:border-gray-700 px-2 py-1 rounded-lg">
                        <Paperclip size={11} /> {attachments.length} anexo{attachments.length > 1 ? "s" : ""}
                ...
                {inv.status === "pending" && (
                    <Form method="post">
                        <input type="hidden" name="intent" value="paid" />
                        <input type="hidden" name="id" value={inv.id} />
                        <button type="submit" className="flex items-center gap-1 text-xs text-green-600 border border-green-200 dark:border-green-900 px-2 py-1 rounded-lg hover:bg-green-50 dark:hover:bg-green-950/30 transition-colors">
                            <CheckCheck size={11} /> Marcar pago
                        </button>
                    </Form>
                )}
                <Form method="post" onSubmit={(e) => !confirm("Eliminar fatura?") && e.preventDefault()}>
                    <input type="hidden" name="intent" value="delete" />
                    <input type="hidden" name="id" value={inv.id} />
                    <button type="submit" className="flex items-center gap-1 text-xs text-red-400 border border-red-200 dark:border-red-900 px-2 py-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors">
...
export default function AdminInvoices() {
    const { invoices, sites, filters, total, pending, attachmentsByInvoice } = useLoaderData<typeof loader>();
    ...
    return (
        <div>
            ...
            <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-5 mb-6">
                <h2 className="font-semibold mb-4 flex items-center gap-2"><PlusCircle size={16} /> Nova Fatura</h2>
                <Form method="post" encType="multipart/form-data" className="space-y-4">
                    <input type="hidden" name="intent" value="create" />
                    {result?.error && (
                        <p className="text-sm text-red-600 bg-red-50 dark:bg-red-950/30 px-4 py-2.5 rounded-lg">{result.error}</p>
                    )}
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                        <div><label className="block text-xs font-medium mb-1.5 text-gray-500">Site / Cliente</label>
                            <select name="siteId" required
                                ...
                                {sites.map((s) => <option key={s.id} value={s.id}>{s.name}{s.owner_name ? ` (${s.owner_name})` : ""}</option>)}
                            </select></div>
                        <div><label className="block text-xs font-medium mb-1.5 text-gray-500">Descrição</label>
                            <input name="description" placeholder="Manutenção mensal" required
                                className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                        <div><label className="block text-xs font-medium mb-1.5 text-gray-500">Valor (€)</label>
                            <input name="amount" type="number" step="0.01" min="0.01" placeholder="0.00" required
                                className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                        <div><label className="block text-xs font-medium mb-1.5 text-gray-500">Data limite (opcional)</label>
                            <input name="dueDate" type="date"
                                className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-gray-700 bg-white dark:bg-gray-800 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" /></div>
                    </div>
                    <div>
                        <label className="flex items-center gap-1.5 text-xs font-medium text-gray-500 mb-1.5"><Paperclip size={12} /> Anexar ficheiro (opcional)</label>
                        <input type="file" name="files" multiple accept=".pdf,.jpg,.jpeg,.png,.webp"
                            className="text-sm text-gray-500 file:mr-3 file:py-1.5 file:px-3 file:rounded-lg file:border-0 file:text-xs file:font-medium file:bg-gray-50 file:text-gray-700 hover:file:bg-gray-100 dark:file:bg-gray-800 dark:file:text-gray-300" />
                        <p className="text-xs text-gray-400 mt-1">PDF, JPG, PNG, WebP — máx. 10 MB por ficheiro</p>
                    </div>
                    <div className="flex justify-end">
                        <button type="submit" className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded-xl transition-colors">Criar Fatura</button>
                    </div>
                </Form>
            </div>
            ...
            {invoices.length === 0 ? (
                ...
                <>
                    ...
                    <div className="md:hidden space-y-3">
                        {invoices.map((inv) => (
                            <InvoiceCard key={inv.id} inv={inv} attachments={attachmentsByInvoice[inv.id] ?? []} />
                        ))}
                    </div>
                    {/* Desktop: tabela */}
