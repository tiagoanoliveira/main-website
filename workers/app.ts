...
/** Lê o corpo do email (text/plain preferido, fallback text/html sem tags) */
...
/** Remove citações e assinaturas, guarda apenas a nova mensagem */
...
/** Processa anexos MIME, faz upload para R2 e devolve metadados */
...
async function notifyAdminEmailReply(env: Env, ticketId: number, clientName: string, clientEmail: string, message: string) {
...
export default {
  // ── Handler HTTP (aplicação React Router) ──
  ...
  // ── Handler de email inbound ──
  // Ativado pelo Cloudflare Email Routing: support@tiagoanoliveira.pt → este worker
  async email(message: ForwardableEmailMessage, env: Env, ctx: ExecutionContext) {
    ...
    if (!ticketId) {
      try { await message.forward(env.ADMIN_EMAIL); } catch (e) {
        console.error("Email sem ticket ID, erro ao reencaminhar:", e);
      }
      return;
    }
    ...
    const ticket = await env.DB
      .prepare(`SELECT id, client_name, client_email, public_token, category FROM tickets WHERE id = ?`)
      .bind(ticketId)
      .first<{ id: number; client_name: string; client_email: string; public_token: string; category: string }>();
    ...
    for (const att of attachments) {
      await env.DB
        .prepare(`INSERT INTO attachments (entity_type, entity_id, file_name, file_type, file_size, r2_key)
    ...
    // Notificar admin
    await notifyAdminEmailReply(env, ticketId, ticket.client_name, fromAddr, body);
  },
